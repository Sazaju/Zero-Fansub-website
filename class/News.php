...
class News {
	...
	public static function getAllNews() {
		if (News::$allNews === null) {
			...
			$message->addLine();
			$message->addLine("* La plupart des pages du site ont été refaites. Le forum, Twitter et les pages d'avancement/recrutement/torrent et les liens du menu des dons ne faisant pas partie du site (gérés à part), il reste les <b>dossiers et galleries d'images</b> à faire, ainsi que les différentes <b>pages de news</b> (en cours, et c'est assez volumineux aussi). Toutes les autres (accueil, contact, équipe, projets, ...) sont faites.");
			$message->addLine();
			$message->addLine("* Le site profite désormais d'un peu de <b>gestion dynamique</b> (contenu changé à la volée). Certaines fonctionnalités peuvent donc être implémentées pour rendre le site un peu plus intelligent, mais encore rien d'extraordinaire vu que les données sont toujours écrites en dur (et non en base de données). le point suivant montre un exemple d'application.");
			$message->addLine();
			$message->addLine("* La section H se retrouve désormais <b>fusionnée</b> à la section tout public, donc il n'y a plus de différence entre les deux. Le passage entre section H et tout public se fait un peu différemment par rapport à avant : désormais vous êtes soit en mode <i>tout public</i>, soit en mode <i>hentai</i>, <b>quelque soit la page</b> (et non certaines pages tout public et d'autres hentai, comme ça se faisait avant). Selon le mode, les projets affichés sont les projets correspondants au mode (la liste habituelle ou les projets H). Pour passer en mode hentai, il suffit de cliquer sur le <b>lien <i>Hentai</i></b> dans le menu de gauche, comme avant. Une fois en mode hentai, ce lien est changé par un <b>lien <i>Tout public</i></b>. Vous aurez compris qu'il suffit de cliquer dessus pour repasser en mode tout public. Lorsque vous passez du mode tout public au mode hentai, un avertissement est affiché (le même qu'avant, pour ceux qui le connaissent {^_^}). Le passage inverse en revanche se fait directement. Si vous demandez d'accéder à un projet hentai alors que vous n'êtes pas en mode hentai (lien direct, lien <i>Voir aussi</i> dans un projet, ...), le passage se fera via l'avertissement aussi. Quand vous refusez de passer en mode hentai, vous retournez à la page précédente (à l'index pour les accès directs).");
			$message->addLine();
