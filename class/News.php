...
class News {
	...
	public static function getAllNews() {
		if (News::$allNews === null) {
			...
			$message->addLine();
			$message->addLine("* TOUTES les pages projets, incluant toutes les releases, ont été refaites. Autrement dit plusieurs centaines de fichiers on été factorisées (c'est ce dont je me réjouis le plus, parce que disposant de tonnes de données et hyper répétitifs, donc le plus chiant à faire). La version 3.2 (que peu ont dû voir passer {^_^}) sonnait le glas du raffinage des releases. Cette nouvelle version sonne celui du raffinage des projets.");
			$message->addLine();
			$message->addLine("* La plupart des pages du site ont été refaites. Le forum, Twitter et les pages d'avancement/recrutement/torrent et les liens du menu des dons ne faisant pas partie du site (gérés à part), il reste les dossiers et galleries d'images à faire, ainsi que les différentes pages de news (en cours, et c'est assez volumineux aussi). Toutes les autres (accueil, contact, équipe, projets, ...) sont faites.");
			$message->addLine();
			$message->addLine("* La section H se retrouve désormais fusionnée à la section tout public, donc il n'y a plus de différence entre les deux (pour ceux qui faisaient attention aux détails). Au passage, le passage entre section H et tout public se fait un peu différemment par rapport à avant : désormais vous êtes soit en mode <i>tout public</i>, soit en mode <i>hentai</i>, quelque soit le page (et non certaines pages tout public et d'autres hentai, comme ça se faisait avant). Selon le mode, les projets affichés sont les projets correspondants (la liste habituelle ou les projets H). Pour passer en mode <i>hentai</i>, il suffit de cliquer sur le lien <i>Hentai</i> dans le menu de gauche, comme avant. Une fois en mode hentai, ce lien est changé par un lien <i>Tout public</i>. Vous aurez compris qu'il suffit de cliquer dessus pour repasser en mode tout public. Lorsque vous passez du mode tout public au mode hentai, un avertissement est affiché (le même qu'avant, pour ceux qui le connaissent {^_^}). Le passage inverse en revanche se fait directement. Si vous demandez d'accéder à un projet hentai alors que vous n'êtes pas en mode hentai, le passage se fera de la même manière (avertissement pour confirmer le passage en mode hentai), et si vous refusez vous serez redirigés vers l'index.");
			$message->addLine();
			$message->addLine("Il me semble que c'est à peu près tout {^_^}. Sur ce, bon leech.");
			...
			$message->addLine();
			$message->addLine("Nous avons appris qu'Ankama va diffuser &agrave; partir de la rentrée de septembre 2011 :");
			$message->addLine("Baccano, Kannagi et Tetsuwan Birdy Decode. Tous les liens on donc été retirés.");
			$message->addLine("On vous invite &agrave; cesser la diffusion de nos liens et &agrave; aller regarder la série sur leur site.");
			$message->addLine();
			...
			$message->addLine("C'est pas souvent qu'on demande du renfort, mais l&agrave;, c'est devenu indispensable...");
			$message->addLine("Nous avons perdu un trad récemment, il ne nous en reste plus qu'un... et comble de malheur,  il n'a pas accroché &agrave; la série, mais je le remercie pour avoir quand m&ecirc;me traduit deux épisodes pour nous dépanner.");
			...
			$message->addLine(".");
			$message->addLine();
			$message->addComponent("Pour postuler, faites une candidatures &agrave; l'école : ");
			...
			$message->addLine("N&#039;hésitez pas &agrave; nous dire ce que vous en pensez dans les commentaires. C&#039;est en apprenant de ses erreurs qu&#039;on avance, après tout ;)");
			$message->addLine();
			...
			$message->addLine("Quelques petits changements dans notre fa&ccedil;on de faire habituelle, on attend vos retours avec impatience ;)");
			$news->setMessage($message);
			News::$allNews[] = $news;
			...
			$message->addLine();
			$message->addLine("On continue dans les séries complètes avec cette fois-ci la petite série de 6 OAV qui fait suite &agrave; la série Tayutama ~ Kiss on my Deity : les 'Pure my Heart'. Ils sont assez courts mais plut&ocirc;t dr&ocirc;le alors amusez-vous bien !");
			$news->setMessage($message);
			News::$allNews[] = $news;
			...
			$message->addLine();
			$message->addLine("Et n&#039;oubliez pas : si vous avez une remarque, une question ou quoi que ce soit &agrave; nous dire, utilisez le système de commentaires ! Nous vous répondrons avec plaisir.");
			$message->addLine();
			$message->addLine("Bons épisodes, &agrave; très bient&ocirc;t pour les 6 OAV supplémentaires Potemayo... et un petit bonjour &agrave; toi aussi !");
			$news->setMessage($message);
			News::$allNews[] = $news;
			...
			$message->addLine("La s&eacute;rie Kujibiki Unbalance 2 a enti&eacute;rement &eacute;t&eacute; refaite !");
			$message->addLine("Les polices illisibles ont &eacute;t&eacute; chang&eacute;es, les panneaux stylis&eacute;s ont &eacute;t&eacute; refait, la traduction a &eacute;t&eacute; revue, bref, une jolie s&eacute;rie compl&egrave;te vous attend !");
			$message->addLine();
			$message->addLine("Pour t&eacute;l&eacute;charger les &eacute;pisodes, c'est comme d'habitude :");
			...
			$message->addLine("Cette s&eacute;rie est comp&eacute;tement ind&eacute;pendante, n'a rien a voir avec la premi&eacute;re saison de Kujibiki Unbalance ni avec la s&eacute;rie Genshiken et il n'est pas n&eacute;cessaire d'avoir vu celles-ci pour appr&eacute;cier cette petite s&eacute;rie.");
			$message->addLine();
			$message->addLine("Si vous avez aim&eacute; la s&eacute;rie, si vous avez des remarques &agrave; nous faire ou autre, n'h&eacute;sitez pas &agrave; nous en faire part ! (Commentaires, Forum, Mail, IRC, ...)");
			$message->addLine();
			$message->addLine("&Agrave; tr&eacute;s bient&ocirc;t pour Potemayo !");
			$news->setMessage($message);
			News::$allNews[] = $news;
			...
			$message->addLine("Rin, Kuro et Mimi sont de retour dans un OAV Sp&eacute;cial de Kodomo no Jikan : Kodomo no Natsu Jikan ! Elles sont toutes les trois absulument adorables dans leurs maillots de bains d'&eacute;t&eacute;, en vacances avec Aoki et Houin.");
			$message->addLine();
			...
			$message->addLine("On peut dire qu'il s'est fait attendre cet Ã©pisode...");
			$message->addLine("Mais le voilÃ  enfin, et c'est tout ce qui compte.");
			$message->addLine("Vous devez vous demander ce qu'il advient de notre annonce de sortie une semaine/un Ã©pisode pour kissxsis.");
			$message->addLine("Vous avez remarquÃ© que c'est un echec. Pourquoi ? Les Ã©pisodes s'avÃ¨rent bien plus longs Ã  rÃ©aliser que prÃ©vu si on souhaite continuer Ã  vous fournir la meilleure qualitÃ© possible. De plus, j'Ã©tais dans ma pÃ©riode de fin d'annÃ©e scolaire et j'ai dÃ» mettre de cÃ´tÃ© nos chÃ¨res soeurs jumelles pour Ãªtre sÃ»re de passer en annÃ©e supÃ©rieure...!");
			$message->addLine();
			$message->addLine("Une nouvelle qui ne vous fera peut-Ãªtre pas plaisir, mais qui j'Ã©spÃ¨re ne vous dÃ©couragera pas de mater les soeurettes un peu plus tard : Nous avons l'intention d'attendre la sortie des Blu-Ray des autres Ã©pisodes avant de continuer KissXsis. La qualitÃ© des vidÃ©os sera meilleure, il y aura moins de censure, plus de dÃ©tails, bref, plus de plaisir !<br />
Le premier Blu-Ray contenant les 3 premiers Ã©pisodes vient tout juste de sortir et nous sortirons bientÃ´t des nouvelles versions de ces trois premiers. Croyez-moi, Ã§a en vaut la peine. Vous ne me croyiez pas ? <a href='http://www.sankakucomplex.com/2010/06/24/kissxsis-erotic-climax-dvd-ero-upgrades-highly-salacious/' target='_blank'>Petit lien</a>.");
			$message->addLine();
			$message->addLine("Et pour ne pas parler que de KissXsis, sachez qu'une petite surprise que je vous ai personnellement concoctÃ© devrait bientÃ´t sortir...<br />
En ce qui concerne les autres projets, nous devrions nous concentrer sur Kujian en attendant les Blu-Ray de KissXsis et boucler certains vieux projets comme Sketchbook, Kodomo no Jikan (le film) ou Tayutama.");
			$message->addLine();
			$message->addLine("En ce qui concerne l'Ã©cole du fansub, elle va trÃ¨s bien et le nombre d'Ã©lÃ¨ve augmente chaque jour, les exercices et les cours aussi ! Si vous Ãªtes intÃ©rÃ©ssÃ©s, vous savez oÃ¹ nous trouver : sur le forum ZÃ©ro fansub.");
			$message->addLine();
			$message->addLine("Bonne chance Ã  ceux qui sont en examens, et que ceux qui sont en vacances en profite bien. Moi, je suis en vacances :p");
			$news->setMessage($message);
			News::$allNews[] = $news;
...
Vous en rêviez ? Les fans l'ont dessiné... Est-ce que c'est ce qui va se passer dans la suite de l'anime ? Ça semble bien parti... Regardez vite l'épisode 10 pour le savoir ! Et comme on a trop envie de savoir la suite à la fin de cet épisode, je vous promets qu'il ne tardera pas.");
			...
			$message = new SimpleTextComponent();
			$message->addLine("<img src='images/news/canaancos.png' /><br />
